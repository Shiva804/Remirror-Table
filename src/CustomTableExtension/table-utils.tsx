...
function getCellAttrs(dom: HTMLElement) {
  ...
  const highlight = dom.getAttribute("highlight");
  return {
    colspan,
    rowspan: Number(dom.getAttribute("rowspan") ?? 1),
    colwidth: widths && widths.length === colspan ? widths : null,
    background: dom.style.backgroundColor || null,
    border: dom.style.border || null,
...
function setCellAttrs(node: ProsemirrorNode) {
  ...
  if (node.attrs.background) {
    attrs.style = `${attrs.style ?? ""}background-color: ${
      node.attrs.background as string
    };`;
  }
  ...
  if (node.attrs.highlight) {
    attrs["highlight"] = node.attrs.highlight;
  }
  ...
  return {
    table: {
      ...
      toDOM(node) {
        return [
          "table",
          ["tbody", { ...extra.dom(node), ...setCellAttrs(node) }, 0],
        ];
      },
    },
    ...
    tableRow: {
      ...
      toDOM(node) {
        return ["tr", { ...extra.dom(node), ...setCellAttrs(node) }, 0];
      },
    },
...
export function createTable(props: CreateTableProps): ProsemirrorNode {
  ...
  for (let ii = 0; ii < columnsCount; ii++) {
    cells.push(
      createCell({ type: tableCell, content: cellContent }) as ProsemirrorNode
    );
...
const { CREATE_COMMAND_DESCRIPTION, CREATE_COMMAND_LABEL } =
  ExtensionTablesMessages;
