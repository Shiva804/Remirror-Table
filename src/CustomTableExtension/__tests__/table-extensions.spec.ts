...
extensionValidityTest(CustomTableExtension);
...
describe("schema", () => {
  const { schema } = createCoreManager(() => [new CustomTableExtension()]);

  const { doc, table, tableRow, tableCell, tableHeaderCell, p } = pmBuild(
    schema,
    {
  ...
  it("parses from html", () => {
    expect(
      htmlToProsemirrorNode({ content: html, schema })
    ).toEqualProsemirrorNode(doc(node));
  });
});

function create() {
  return renderEditor([new CustomTableExtension()]);
}
...
describe("commands", () => {
  const setup = () => {
    ...
    const build = (...rows: string[][]) => {
      ...
      return table(
        ...rows.map((r) => row(...r.map((content) => cell(p(content)))))
      );
    };
